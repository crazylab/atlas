...
@Component("aadAuthenticationFilter")
public class AtlasAADSSOAuthenticationFilter implements Filter {
    private static final Logger LOG = LoggerFactory.getLogger(AtlasAADSSOAuthenticationFilter.class);

    public static final String BROWSER_USERAGENT = "atlas.sso.aad.browser.useragent";
    public static final String JWT_AUTH_PROVIDER_URL = "atlas.sso.aad.providerurl";
    public static final String JWT_PUBLIC_KEY = "atlas.sso.aad.publicKey";
    public static final String JWT_COOKIE_NAME = "atlas.sso.aad.cookiename";
    public static final String JWT_ORIGINAL_URL_QUERY_PARAM = "atlas.sso.aad.query.param.originalurl";
    public static final String JWT_COOKIE_NAME_DEFAULT = "aad-jwt";
    ...
    private String cookieName = "aad-jwt";
    ...
    @Inject
    public AtlasAADSSOAuthenticationFilter(AtlasAuthenticationProvider authenticationProvider) {
        ...
        if (configuration != null) {
            ssoEnabled = configuration.getBoolean("atlas.sso.aad.enabled", false);
    ...
    /*
     * doFilter of AtlasAADSSOAuthenticationFilter is the first in the filter list so in this it check for the request
     * if the request is from browser and sso is enabled then it process the request against aad sso
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("AAD doFilter {}", httpRequest.getRequestURI());
        }
...
//        }
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("AAD ssoEnabled  {} {}", ssoEnabled, httpRequest.getRequestURI());
        }
        ...
        // if we get the aad-jwt token from the cookies then will process it further
        if (serializedJWT != null) { // TODO: Come back for validation flow
...
//                        List<GrantedAuthority> grantedAuths = AtlasAuthenticationProvider.getAuthoritiesFromUGI(userName);
                        ...
                        grantedAuths.add(new SimpleGrantedAuthority("ROLE_ADMIN")); //TODO: Change Always admin
                ...
                } else {  // if the token is not valid then redirect to aad sso
                    redirectToAAD(httpRequest, httpServletResponse, filterChain);
                }
    ...
    private void redirectToAAD(HttpServletRequest httpRequest, HttpServletResponse httpServletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        if ("XMLHttpRequest".equals(ajaxRequestHeader)) {
            ...
            json.put("aadssoredirectURL", URLEncoder.encode(ssourl, "UTF-8"));
...
//        }
        ...
        StringBuilder aadLoginURL = new StringBuilder();

        aadLoginURL.append(authenticationProviderUrl);
...
//                safeAppend(aadLoginURL, xForwardedURL, atlasApplicationURL);
//            } else {
//                safeAppend(aadLoginURL, atlasApplicationURL);
//            }
//        } else {
        if (StringUtils.trimToNull(xForwardedURL) != null) {
            safeAppend(aadLoginURL, xForwardedURL, getOriginalQueryString(request));
        } else {
            safeAppend(aadLoginURL, request.getRequestURL().toString(), getOriginalQueryString(request));
        }
//        }
        return aadLoginURL.toString();
    }
    ...
    public SSOAuthenticationProperties loadJwtProperties() {
        ...
        if (providerUrl != null && configuration.getBoolean("atlas.sso.aad.enabled", false)) {
            SSOAuthenticationProperties jwtProperties = new SSOAuthenticationProperties();
            String publicKeyPathStr = configuration.getString(JWT_PUBLIC_KEY); // TODO: Check usage
